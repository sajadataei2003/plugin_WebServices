        ...
        if ( is_array( $options ) && ! empty( $options ) ) {
          ...
          foreach ( $options as $option_key => $option_value ) {
            ...
            if ( is_array( $option_value ) && ! empty( $option_value ) ) {
              ...
              $data = apply_filters( 'inboxino_csf_checkbox-data', '', $option_key ); 
              $status = apply_filters( 'inboxino_csf_checkbox-status', '', $option_key );
              echo '<li>';
              echo '<label>';
              echo '<input type="checkbox" name="'. esc_attr( $this->field_name( '[]' ) ) .'" value="'. esc_attr( $option_key ) .'"'. $this->field_attributes() . esc_attr( $checked ) . $data . '/>';
