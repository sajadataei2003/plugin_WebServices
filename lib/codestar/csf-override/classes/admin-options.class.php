...
if ( ! class_exists( 'CSF_Options' ) ) {
    class CSF_Options extends CSF_Abstract {
        ...
        public function add_options_html() {
            ...
            if ( apply_filters( "inboxino_csf_{$this->unique}_alt_notification", true ) ) {
                $notice_class = ( ! empty( $this->notice ) ) ? apply_filters( "inboxino_csf_{$this->unique}_notice_class" , 'csf-form-show' ) : '';
                $notice_text  = ( ! empty( $this->notice ) ) ? $this->notice : '';

                $message_class = apply_filters( "inboxino_csf_{$this->unique}_message_class", 'csf-form-success' ); 
                echo '<div class="csf-form-result ' . $message_class . ' '. esc_attr( $notice_class ) .'">'. $notice_text .'</div>';
            }
            ...
            $submit_labels = array( 'value' => esc_html__( 'Save', 'csf' ), 'data-save' => esc_html__( 'Saving...', 'csf' ) );
            $submit_labels = apply_filters( "inboxino_csf_{$this->unique}_submit_label", $submit_labels );

            echo '<div class="csf-buttons">';
            echo '<input type="submit" name="'. esc_attr( $this->unique ) .'[_nonce][save]" class="button button-primary csf-top-save csf-save'. esc_attr( $ajax_class ) .'" value="'. $submit_labels['value'] .'" data-save="'. $submit_labels['data-save'] .'">';
            ...
            if ( apply_filters( "inboxino_csf_{$this->unique}_alt_notification", false ) ) {
                $notice_class = ( ! empty( $this->notice ) ) ? apply_filters( "inboxino_csf_{$this->unique}_notice_class" , 'csf-form-show' ) : '';
                $notice_text  = ( ! empty( $this->notice ) ) ? $this->notice : '';

                $message_class = apply_filters( "inboxino_csf_{$this->unique}_message_class", 'csf-form-success' ); 
                echo '<div class="csf-form-result ' . $message_class . ' '. esc_attr( $notice_class ) .'" style="float:unset; margin-right:unset;padding:10px; font-size: 1rem;">'. $notice_text .'</div>';
            }
            ...
            if ( $has_nav ) {
                ...
                $additional_tabs = apply_filters( "inboxino_csf_{$this->unique}_tabs", [] );
                foreach ( $additional_tabs as $tab ) {
                    echo '<li class="csf-tab-item"><a href="' . esc_url( $tab['url'] ) . '">'. $tab['title'] .'</a></li>';
                }
            ...
            if ( ! empty( $this->args['show_footer'] ) ) {
                ...
                echo '<div class="csf-buttons">';
                echo '<input type="submit" name="csf_transient[save]" class="button button-primary csf-save'. esc_attr( $ajax_class ) .'" value="'. $submit_labels['value'] .'" data-save="'. $submit_labels['data-save'] .'">';
                echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( 'Reset Section', 'csf' ) .'" data-confirm="'. esc_html__( 'Are you sure to reset this section options?', 'csf' ) .'">' : '';
                echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( 'Reset All', 'csf' ) : esc_html__( 'Reset', 'csf' ) ) .'" data-confirm="'. esc_html__( 'Are you sure you want to reset all settings to default values?', 'csf' ) .'">' : '';
                echo '</div>';
